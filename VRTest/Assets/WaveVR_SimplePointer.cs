...
public class WaveVR_SimplePointer : MonoBehaviour
{
    ...
    private GameObject pointerHolder;//射线起点（cube与sphere的父节点）
    ...
    void InitPointer()
    {
        pointerHolder = new GameObject(string.Format("[{0}]Holder", gameObject.name));
        pointerHolder.transform.parent = transform;
        pointerHolder.transform.localPosition = Vector3.zero;

        pointer = GameObject.CreatePrimitive(PrimitiveType.Cube);
        pointer.transform.name = string.Format("[{0}]Pointer", gameObject.name);
      ...
      void Update()
    {
        if (pointer.gameObject.activeSelf)
        {
            ...
            if (WaveVR_Controller.Input(type).GetPressDown(WVR_InputId.WVR_InputId_Alias1_Trigger))
            {//按下triger键
    ...
    void NotifyEvent(GameObject go, string funcName, object obj)
      ...
      public Transform GetHitTrans()//碰撞到物体信息
      {
          return detalhititem;
      }
      public Vector3 GetHitPos()//碰撞点的坐标
      {
          return detalhitpos;
      }
